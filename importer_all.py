...
def import_municipios():
    ...
    TABLE_NAME = settings.TABLE_GEOM_MUNICIPIOS
    CSV_PATH = os.path.join('datasets', 'municipios_ceara.csv')
    SRID = getattr(settings, "GEOM_MUNICIPIOS_SRID", 4326)  # Alterado para 4326
            ...
            with engine.begin() as conn:
                insert_sql = text(f"""
                    INSERT INTO {TABLE_NAME} (
                        ...
                        :cd_uf, :nm_uf, :cd_regiao, :nm_regiao, :cd_concurb, :nm_concurb,
                        :area_km2, 
                        -- Convertendo de SRID 4674 (do CSV) para 4326 (armazenamento)
                        ST_Transform(
                            ST_SetSRID(ST_GeomFromEWKB(decode(:geometry, 'hex')), 4674),
                            4326
                        )
                    )
                    ON CONFLICT (cd_mun) DO UPDATE SET
...
def main():
    ...
    for name, result in results.items():
        ...
        if status == 'success':
            ...
            print(f"{name}: ✓ SUCESSO (Registros: {stats.get('registros_salvos', 'N/A')})")
        elif status == 'skipped':
            print(f"{name}: ⤷ NÃO NECESSÁRIO ({result.get('reason', 'Dados recentes')})")
        else:
            print(f"{name}: ✗ ERRO ({result.get('message', 'Erro desconhecido')})")
    print("======================================")
